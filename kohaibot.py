...
class Main:
    ...
    #osu Commands
            ...
            elif message.content.startswith('$bsay'):
                if 'SynSenpai#5410' in str(message.author):
                    print('+'*10)
                    print('Server listing')
                    for i in range(0, len(config_list)):
                        print('{0}:{1}'.format(config_list[i], i))
                    server = config_list[int(input('Input server number: '))]
                    talkmode = True
                    print('+'*10)
                    print('Now entering talk mode')
                    print('+'*10)
                    while talkmode:
                        msg = input('Message: ')
                        if msg == 'stopsay':
